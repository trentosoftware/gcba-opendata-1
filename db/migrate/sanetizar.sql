...
delete from parcelas_geometry where char_length(substring(parcela from '[^\w]+')) > 0 or char_length(substring(manzana from '[^\w]+')) > 0;
...
DELETE from parcelas_geometry where char_length(seccion) != 3;
